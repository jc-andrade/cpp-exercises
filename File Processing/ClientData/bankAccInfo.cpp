...
/* This program reads a random-access file sequentially, Updates data previouly written to the file
   Creates data to be placed in the file, and then deletes data previously stored in the file  */
...
int getAccount(const char * const); // Constant pointer to a constant char reference
...
int main()
{
	...
	// ennable user to specify action
...
void newRecord(fstream &insertInFile)
{
	...
	if (client.getAccountNumber() == 0)
	{
		...
		// user enterse last name, first name and balance
		cout << "Enter last name, first name, balance\n? ";
		cin >> setw(15) >> lastName;
		cin >> setw(10) >> firstName;
		cin >> balance;

		// use valueto populate account values
...
void deleteRecord(fstream &deleteFromFile)
{
	...
	// delete record, if record exists on file
	if (client.getAccountNumber() != 0)
	{
	...
	else // display error if record does not exists
...
void outputLine(ostream &output, const ClientData &record)
{
	output << left << setw(10) << record.getAccountNumber() 
		<< setw(16) << record.getLastName() << setw(11)
		<< record.getFirstName() << setw(10) << setprecision(2) 
